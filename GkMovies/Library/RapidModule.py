from app import db
from app.models import ImdbMovie, ImdbPlot, UserMovie, PersonMovie, UrlMovie, Person, UrlPerson, Genre, GenreMovie
...
from Library.HelperModuleF import makeDate, getImageUrl, Dget, getItemList
import requests
import time
...
def getID(result):
    ...
    if isinstance(result, str):
        ...
        if 'id' in result:
            ...
            error('No id found in result=' + str(result))
            return None
...
def getByDisplay(cuser, showMine):
    rtn = []
    if showMine == 'False':
        rs = UserMovie.query\
            .filter(and_(UserMovie.user_type == cuser.user_type, UserMovie.user_id == cuser.user_id))\
            .order_by(UserMovie.imdb_movie_id)\
            .all()

        for row in rs:
            rtn.append(row.imdb_movie_id)
            
    return rtn
    

def getRankSql(rankField, checked):
    having = ''
    
    sql = "SELECT im.id, im.tt, title, iyear, imdbTopRank, imdbPopular, outline, group_concat(code ORDER BY gm.srt) as codes, group_concat(genre) as genres, poster\n"
    sql += "FROM imdb_movie im\n"
    sql += "LEFT JOIN imdb_plot ip on im.tt = ip.tt\n"
    sql += "LEFT JOIN genre_movie gm on im.tt = gm.tt\n"
    sql += "LEFT JOIN genre g on gm.genre = g.name\n"
    

    
    if rankField == 'rank':
        where = "WHERE imdbTopRank between 1 and 250\n"
        order = "ORDER BY imdbTopRank;\n"
    else:
        where = "WHERE imdbPopular > 0\n"
        order = "ORDER BY imdbPopular;\n"
        
    if checked != 'all,':
        checked = checked.replace(',', '%%')
        having = "HAVING codes LIKE '%%" + checked + "%%'\n"

    group = "GROUP BY tt\n"
    
    sql += where + group + having + order
    return sql
...
class Rapid(Constants):
    ...
    def listMoviesByGenreRankPopularity(self, cuser, rankField, showMine, checked):
        movies = []

        mineList = getByDisplay(cuser, showMine)
        
        sql = getRankSql(rankField, checked)
        ...
        for row in rs:
            ...
            if rankField == 'rank':
                urlMovie.seq = row['imdbTopRank']
            else:
                urlMovie.seq = row['imdbPopular']
            
            urlMovie.tt = row['tt']
            urlMovie.title = row['title']
            urlMovie.iyear = row['iyear']
            urlMovie.plot = row['outline']
            urlMovie.genres = getItemList(row['genres'])
            urlMovie.poster = row['poster']
            self.setMoviePersonsDb(urlMovie, row['tt'])
            stop = 1

            
        return movies
        
    def getGenreNamesFromCodes(self, checked):
        ...
        for genre in genres:
            if genre.code in checked:
                rtn += " & " + genre.name
        

        return rtn[3:len(rtn)];
    ...
    def getMoviePersons(self, jlist, tt, role): 
        info('jlist size=' + str(len(jlist)) + ', tt=' + tt + ', role=' + role)
    ...
    def getNoIdEndPoint(self, endPoint):
        url = URL + endPoint
        r = requests.request("GET", url, headers=headers)
