...
from app.models import User, UserMovie, UserLog, ImdbMovie, ImdbPlot, UserColumn,\
    load_imdb_movie, load_user
import os
import requests
from app import db
...
class UpdateRelease():
    ...
    def update(self, version, step):
        ...
        if version == '1.5':
            return self.update_1_5(step)

    def update_1_5(self, step):
        if step == '1':
            return self.manageUserColumns_1_5_1()
        elif step == '2':
            return self.insertAddDates_1_5_2()

    def manageUserColumns_1_5_1(self):
        ...
        return 'Done Step 1 Manage User Columns'


    def insertAddDates_1_5_2(self):
        updates = 0
        total = 0
        userMovies = UserMovie.query.filter(UserMovie.dateAdded == '0000-00-00')\
            .order_by(UserMovie.user_id.desc())\
            .order_by(UserMovie.imdb_movie_id.desc()).all()
            
        for userMovie in userMovies: 
            total += 1
            user = load_user(userMovie.user_id)
            if not user:
                continue
                 
            imdbMovie = load_imdb_movie(userMovie.imdb_movie_id)
            
            userLog = UserLog.query.filter(UserLog.login == user.login,\
                                           UserLog.route == 'addMovie',\
                                           UserLog.action.like('tt%'),\
                                           UserLog.arg.like(imdbMovie.tt + '%')).order_by(UserLog.log_time.desc()).first()
                                           
            if not userLog:
                continue
            
            userMovie.dateAdded = userLog.log_time
            updates += 1
            
        db.session.commit()
        
        return 'Done Step 1.5.2 - Insert Add Dates.  Updates = ' + str(updates) + ' out of ' + str(total)
...
where update_date is null and insert_date < '2020-01-15 00:00:00' 
order by user_id desc, iyear desc, title; ''' 
        ...
        for row in rs:
            ...
            if not imovie.update_date:
                adder.updateImdbMovie(imovie, True, True, True)
                cnt += 1
                if cnt >= 20:
                    break
